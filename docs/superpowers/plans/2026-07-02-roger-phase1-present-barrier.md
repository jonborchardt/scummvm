...
- **Fault-injection matrix** (2026-07-03 initial; re-verified 2026-07-03 after contradiction, scratch edits, never committed):
  ...
  Gate detection summary: **ESCALATION CONFIRMED — gate is blind to this fault class.** All injections A/B/C/D green. The minimum exit criterion ("at least one injection red") is NOT met even when all three marks are removed simultaneously (D). The bitsShow hook (`onNativeShowRect`), the sprite dirty path, and the barrier's dirty accumulator together keep the display correct — the `markNativeDirty`/`markVacatedDirty` calls in the erase/clearToken/show paths are architecturally correct belt-and-suspenders coverage but not structurally load-bearing for any scenario the current gate exercises. Phase 3 deletion candidates: all three marks (pending the user's interactive soak confirming no visual regression). Escalation to human stands; no exit criterion change without human approval.
