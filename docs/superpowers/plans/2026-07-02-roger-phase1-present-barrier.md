...
- **Fault-injection matrix** (2026-07-03 initial; re-verified 2026-07-03 after contradiction, scratch edits, never committed):
  ...
  | B | Comment out `markVacatedDirty(removedRects[i]);` loop body in `uiClearToken` | OK | **GREEN** (33/33) | Stayed green — but see the corrected interpretation below: green here does NOT prove `markVacatedDirty` is redundant (the capture path forces a full recompose, masking any missing-mark fault). Not evidence for deletion. |
  ...
  | D | Comment out ALL THREE: `markNativeDirty(nativeRect)` in `onNativeEraseRect`, `markVacatedDirty(removedRects[i])` in `uiClearToken`, AND `markNativeDirty(screenRect)` at the top of `onNativeShowRect` | OK | **GREEN (33/33)** — run 1; run 2: `sq3-walk-perf perf FAIL busy=6/4` (noise — busy jitter, re-run 3 was green); run 3: GREEN | All three marks removed simultaneously: all functional checks (same, presence, exists) passed all three runs. The single busy-count failure in run 2 (busy=6 vs threshold 4, n=170) is within the documented sq3 busy=4–8 ms machine variance (Phase 0 allowance); 2/3 runs fully green. The gate is blind to removal of the FULL mark set. |

  Gate detection summary: **ESCALATION CONFIRMED — gate is blind to this fault class.** All injections A/B/C/D green. The minimum exit criterion ("at least one injection red") is NOT met even when all three marks are removed simultaneously (D). **Interpretation (corrected):** D's green does NOT establish that the marks are redundant. The gate is blind *by construction* — its capture path forces a full clean recompose (`needFullSource` in `presentBarrier`/composite), so a missing-mark fault (which only manifests as a stale region in the *incrementally* seeded scratch) cannot appear in ANY capture. D's green is therefore equally consistent with the marks being load-bearing for on-screen correctness as with their being redundant; the data cannot distinguish the two. These three marks are **unverifiable with the current gate**, NOT confirmed deletion candidates. Before any Phase 3 deletion, add an overlay-truth capture mode (dump the presented scratch / `grabOverlay` WITHOUT forcing full source) and re-run injections A–D against it; only a red there would justify calling any mark redundant. Escalation to human stands; no exit criterion change without human approval.

- **Grow-workaround outcome:** Removed cleanly — bitsRestore's exact erase rect covers the compositor overdraw ring (verified by interactive soak, NOT provable by the capture-based gate, which forces a full recompose); the `n.grow(2)` / `d.grow(2)` fallback was not needed after Task 5's `markNativeDirty` hook in `onNativeEraseRect` was wired. `uiVacatedExtent` (the exact-only math, no native grow) is what `markVacatedDirty` now calls.
