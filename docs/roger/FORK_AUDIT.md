...
| `backends/events/emscriptensdl/emscriptensdl-events.h` | injected-event queue (`injectKey`/`_injected`) draining ahead of the SDL poll, fed by the shell's hidden-input text bridge via the `EmscriptenKbd_pushKey` KEEPALIVE shim; `g_emscriptenKbdSource` pointer as the shim's entry point | web-only backend (never compiled for desktop); same category as the existing phantom-key-repeat/TEXT_INPUT filters in this file. `g_emscriptenKbdSource` is declared `extern` in this header and defined once in `backends/platform/sdl/emscripten/emscripten.cpp`, ensuring every translation unit shares the single live pointer |
| `backends/events/emscriptensdl/emscripten-keynorm.h` (added 2026-07-15) | pure key-normalization helper `injectKey` routes through: control keys carry their ASCII (SCI0 drops ascii-0 keys, which silently ate OSK backspace), uppercase maps to lowercase keycode + `KBD_SHIFT`, smart punctuation maps to ASCII, unmappable >Latin-1 input is dropped; unit-tested (`test/sci/roger/test_emscripten_keynorm.h`, 8 tests) | web-only backend; SDL-free header, so it compiles in the Windows test harness. Strengthens the `kFeatureVirtualKeyboard` upstream-PR candidate (engine-agnostic `Common::KeyCode` contract); an upstream slice would relocate its test out of `test/sci/roger/` (placed there downstream because that is the harness's registered glob) |
