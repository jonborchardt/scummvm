...
* '''GUI''': all gui-related
* '''DEVTOOLS''': ScummVM tools module or built-in tools
...
* '''JANITORIAL''': Cross-subsystem cleanups and formatting changes (non-functional only)
...
* '''Features''' - Brand new features, for example Cloud Integration
