...
struct Sprite {
	...
	uint32 seq = 0;        // stamp seq tag (rollback scope): 0 = pre-phase/non-rollback stamp; >0 = set by processForegroundCaptures, consumed by onRestore.
...
// static first, then animate, then a STABLE sort by ascending priority. Stable =>
// at equal priority addToPic draws before animate (native bakes addToPic into the
// pic first). n is small (a handful of sprites), so insertion sort is fine.
...
// collapses to skip/copy. Painting is CLIPPED to the dest surface and (when given) to
// `clip`, but the source is always sampled against the FULL destRect -- a destRect
// extending past the clip is CROPPED, never compressed (native SCI port clipping,
// and how sprites exit the screen edge without squishing).
...
// S3.3 support: grow `regions` (coalesced, clamped to `bounds`) to a fixpoint over
...
public:
	...
	void addDirtyRect(const Common::Rect &destRect) {
		if (!destRect.isEmpty())
			_dirtyCur.push_back(destRect);
	}
	...
	void addSceneDirtyRect(const Common::Rect &destRect) {
		if (!destRect.isEmpty())
			_sceneDeferredDirty.push_back(destRect);
	}
	...
	// S3.3 region-bounded recompose. Patch `composite` (the persistent scene+UI
	...
	// via a real transition. claimTransition()'s composeRoomScene() pre-warms _bgCache, which
	...
	// coalesce(_sceneDirtyCur union _sceneDirtyPrev). The provider uses this to bound its
	// own per-frame scene-cache copies to the regions that actually changed.
...
private:
	...
	RogerCapabilities _caps;   // set by setCapabilities(); read by render methods
	...
	// as the scene is composited; presentToOverlay pushes _dirtyCur union _dirtyPrev (last
