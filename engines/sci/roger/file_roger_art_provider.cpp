...
FileRogerArtProvider::FileRogerArtProvider(const Common::String &gameId,
	...
	// Overlay-truth captures: when on, a pending
	...
	// Cycle-diff backstop net: at the onFrameEnd seam, diff
	...
	// Default ON; roger_diff_net=false is the runtime escape hatch.
	{
...
void FileRogerArtProvider::pushHiresBackgroundInternal(GuiResourceId pictureId) {
	...
	if (_journal)
		_journal->clear();
	for (uint i = 0; i < _uiIcons.size(); i++) {
		_uiIcons[i]->free();
		delete _uiIcons[i];
	}
	_uiIcons.clear();
	...
	if (!_capsProbed) {
		...
		_capsProbed = true;
		_statusBarH = _caps.statusBarRows;
		if (_debugLog)
...
void FileRogerArtProvider::presentWithUi() {
	...
	if (_compositeCacheValid && !needFullSource && _mode != Roger::kModeSideBySide) {
		// sec.3.3 region-bounded recompose: patch the composite cache only inside the
...
void FileRogerArtProvider::markNativeDirty(const Common::Rect &nativeRect) {
	// sec.3.1 exact invalidation: SCI touched these native pixels. grow(1) native
...
void FileRogerArtProvider::presentBarrier() {
	// The single gated present. Every skip path below is O(1).
	if (_inAnimateCycle)
...
// Dispatch onControl by kind to the per-kind bodies below.
...
// Relaxes the native kernelTexteditChange pixel-width keystroke cap. The hires
...
// applies, so nothing overflows. observer-null / disabled keeps the native cap.
...
void FileRogerArtProvider::onDrawCelInternal(const Common::Rect &r, int viewId, int loopNo, int celNo) {
	...
	// room change -- never via a blanket namespace clear.
...
void FileRogerArtProvider::uiPushStatusInternal(const Common::Rect &r, const char *text, int fontId,
	...
	// how the backend samples its game blit.
...
void FileRogerArtProvider::menuRebuildBar() {
	...
	// whole reserved strip stays overlay-owned.
...
void FileRogerArtProvider::onWindowClose(uint32 token) {
	...
	// op captured inside it (controls + generic text). Singleton tokens (status strip
	// 0x10.., dropdown 0x20..) from menu.cpp route through the same clear path.
	clearWindowToken(token);
}

void FileRogerArtProvider::clearWindowToken(uint32 token) {
	// Sole caller is onWindowClose dispatcher; tokens seen:
	...
	if (_journal) {
		...
		if (ns == Roger::kControlTokenNs) {
			...
			removedUi = false; // lifetime is bracket/erase-based; nothing to clear here
		} else {
	...
	// no-save-under / reanimate==false disposals never fire bitsRestore, and Feeder B
	// stamp rects can exceed the save-under rect -- this is their only same-present
	// invalidation (net blind while frozen; _dirtyPrev a frame late).
...
void FileRogerArtProvider::onErase(const Common::Rect &nativeRect) {
	if (!enabled)
		return;
	// sec.3.1 exact invalidation: the restored save-under rect, straight from SCI.
...
void FileRogerArtProvider::journalAppend(const Roger::UiElement &e) {
	...
	// (a timing-fragile any-intersection rule would cancel valid reveals).
...
void FileRogerArtProvider::onRestore(uint32 handleToken, const Common::Rect &rect) {
	if (!enabled)
		return;
	// sec.3.1 invalidation first, exactly like onErase (the barrier defers
...
void FileRogerArtProvider::uiPushFrameBoxInternal(const Common::Rect &r, int penColor) {
	...
	// Per-cycle discipline: only mutate + present when the frame changed.
...
void FileRogerArtProvider::onShowInternal(const Common::Rect &screenRect, uint32 ownerToken) {
	// sec.3.1 exact invalidation: SCI showed these native pixels, so the overlay
	...
	_foregroundRegions.push_back(r);
}
...
void FileRogerArtProvider::onAnimateFrame(const AnimateList &list) {
	...
	presentBarrier(); // per-cycle present (fresh-frame branch -- no recompose)
...
// -- DEBUG TOOL: in-game quick-tune panel ---
...
void FileRogerArtProvider::onPictureAbsent() {
	// Reserved companion to onPicture: the caller emits it where a
...
bool FileRogerArtProvider::claimShake(int shakeCount, int directions) {
	...
	if (!_transitionsEnabled || _mode == Roger::kModeSideBySide ||
		...
		return true; // side-by-side: pure FX, would present the non-split layout -- skip
	const int OW = g_system->getOverlayWidth(), OH = g_system->getOverlayHeight();
	if (OW <= 0 || OH <= 0 || _sceneCache->w != OW || _sceneCache->h != OH)
		return true;
