...
| File | Role |
|------|------|
| `engines/sci/roger/roger_art_provider.h` | Abstract interface + `g_sciRogerProvider` global; declares no-op base virtuals for native-extras: `onAddToPicCel`, `beginNativeDraw`, `endNativeDraw`, `onNativeShowRect`, `snapshotNativeBaseline` |
...
| `engines/sci/graphics/{paint16,controls16,menu,event}.cpp` | Hook sites: picture replace, dialog/control capture, status/menu bar, F10 toggle; `paint16.cpp` also hosts the `bitsShow` hook (`onNativeShowRect`) and `beginNativeDraw`/`endNativeDraw` re-entrancy guards (Feeder B) |
