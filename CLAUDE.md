...
| `engines/sci/roger/ui/roger_widgets.{h,cpp}` + `roger_panel_style.{h,cpp}` | Shared panel UI kit: PanelWidget + packed-id hit-testing; PanelStyle palette, PanelFonts (TTF roles + bitmap fallback), PanelPainter -- the picker, pass builder, F12 tune panel, and Studio all draw through it |
