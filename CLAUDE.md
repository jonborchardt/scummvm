...
| `backends/platform/sdl/emscripten/emscripten.{h,cpp}`, `backends/events/emscriptensdl/emscriptensdl-events.h` (mobile additions), `dists/emscripten/assets/manifest.json`, `dists/emscripten/custom_shell.html` (mobile-keyboard/viewport additions) | — | **Web-demo mobile support (downstream; native Emscripten/wasm build).** `emscripten.{h,cpp}` answers the `kFeatureVirtualKeyboard` hook (focuses/blurs a hidden shell `<input>` to summon the phone OSK — the `kFeatureVirtualKeyboard` implementation itself is a clean, Roger-agnostic upstream-PR candidate, same category as `http-fs.cpp` above); `emscriptensdl-events.h` adds an injected-event queue (`EmscriptenSdlEventSource::injectKey`/`_injected`, drained ahead of the normal SDL poll) so `custom_shell.html`'s hidden-input text bridge can turn phone-OSK keystrokes into ScummVM key events via the `EmscriptenKbd_pushKey` KEEPALIVE C shim; `manifest.json` adds `"orientation": "landscape"`; `custom_shell.html` adds the mobile viewport (`user-scalable=no`, `touch-action: none`), the rotate-portrait hint, and the hidden-input bridge markup/script. See `docs/roger/WEB_DEMO.md`'s "Mobile support" section for the full flow and the Android acceptance checklist. |
