...
| `event.cpp` + `gui/EventRecorder.h` | ~35 | **Separately pitchable upstream PR** — the `.rin` input driver is a generic headless scripted-input facility complementing EventRecorder; deliberately engine-agnostic (keep it that way). Note: these two files are unrelated changes — `event.cpp` carries observer-seam event hooks (E1–E4), while `gui/EventRecorder.h` carries only the `USE_IMGUI` declaration un-gate (FORK_AUDIT W2). The `.rin` driver itself lives under `engines/sci/roger/` with provider-internal registration; an upstream input-driver PR would need a new neutral registration seam authored as new work. |
...
| `backends/fs/emscripten/http-fs.cpp`, `backends/events/emscriptensdl/emscriptensdl-events.h`, `dists/emscripten/{build-assemble_demo.sh, build-package_game.sh, custom_shell.html, custom_shell-pre.js, roger-demo.ini}` | — | **Web-demo enablement (downstream; native Emscripten/wasm build).** `http-fs.cpp` = page-relative data-dir fetch so a GitHub-Pages subpath deploy works (Roger-agnostic portability fix, the one clean upstream candidate); `emscriptensdl-events.h` = SDL3 phantom key-repeat + `TEXT_INPUT` dedup filters (web-only backend, never compiled for desktop); `dists/emscripten/*` = demo assembly/packaging scripts + landing shell + `roger-demo.ini` (boots Betrayed Alliance). Full build/deploy/redeploy story: `docs/roger/WEB_DEMO.md`. Added after the FORK_AUDIT §2 baseline (see its §12 addendum). |
| `backends/platform/sdl/emscripten/emscripten.{h,cpp}`, `backends/events/emscriptensdl/emscriptensdl-events.h` (mobile additions) + `emscripten-keynorm.h`, `dists/emscripten/assets/manifest.json`, `dists/emscripten/custom_shell.html` (mobile-keyboard/viewport additions) | — | **Web-demo mobile support (downstream; native Emscripten/wasm build).** `emscripten.{h,cpp}` answers the `kFeatureVirtualKeyboard` hook (focuses/blurs a hidden shell `<input>` to summon the phone OSK, change-gated so repeated text-edit redraws don't reset the bridge — the `kFeatureVirtualKeyboard` implementation itself is a clean, Roger-agnostic upstream-PR candidate, same category as `http-fs.cpp` above); `emscriptensdl-events.h` adds an injected-event queue (`EmscriptenSdlEventSource::injectKey`/`_injected`, drained ahead of the normal SDL poll) so `custom_shell.html`'s hidden-input text bridge can turn phone-OSK keystrokes into ScummVM key events via the `EmscriptenKbd_pushKey` KEEPALIVE C shim; `emscripten-keynorm.h` (2026-07-15) is the pure helper `injectKey` normalizes through — control keys get their ASCII (SCI0 drops ascii-0 keys, which ate OSK backspace), uppercase → lowercase keycode + `KBD_SHIFT`, smart punctuation → ASCII, unmappable >Latin-1 dropped — unit-tested via `test/sci/roger/test_emscripten_keynorm.h`; `manifest.json` adds `"orientation": "landscape"`; `custom_shell.html` adds the mobile viewport (`user-scalable=no`, `touch-action: none`), the rotate-portrait hint (touch devices via `pointer: coarse`), the action bar/D-pad, and the hidden-input bridge markup/script. See `docs/roger/WEB_DEMO.md`'s "Mobile support" section for the full flow and the Android acceptance checklist. |
